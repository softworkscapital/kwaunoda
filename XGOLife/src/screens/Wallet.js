...
const Wallet = () => {
  ...
  const getTotalBalances = async () => {
    try {
      const resp = await fetch(
        `${APILINK}/topUp/get_all_entity_total_balances`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const getUserBalances = async (clientProfileId) => {
    try {
      const resp = await fetch(
        `${APILINK}/topUp/userBalance/${clientProfileId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddTopUp = async (e) => {
    ...
    try {
      ...
      let amount = debit; // Fallback to an empty string
      ...
      const response = await initiatePayment(topupPese);
      if (response) {
        ...
        <View style={styles.modalContainer}>
          ...
          <View
            style={[
              styles.topBar,
              styles.goldenYellow,
              { paddingTop: 10, alignItems: "center", marginBottom: "40%" },
            ]}
          >
            <View style={[styles.topBarContent]}>
              <Text
...
const styles = StyleSheet.create({
  ...
  picker: {
    height: 58, // Height for the Picker
