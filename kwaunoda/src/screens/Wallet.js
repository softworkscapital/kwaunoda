...
import { StyleSheet, View, Text, TextInput, TouchableOpacity, Linking, FlatList, Image, Modal, Alert } from 'react-native';
...
const Wallet = () => {
  ...
  const initiatePayment = async (paymentDetails) => {
    try {
        ...
        if (data.success && data.redirectUrl) {
            // Redirect user to the payment URL (Pesepay interface)
            Linking.openURL(data.redirectUrl);
        } else {
            Alert.alert('Payment Error', data.message || 'Failed to initiate payment.');
        }
    } catch (error) {
  ...
  const handleAddTopUp = async (e) => {
    ...
    const userObj = { currency, exchange_rate, date, credit, debit: amount, balance, description, client_profile_id };
    ...
    await initiatePayment(topupDetails); // Initiate payment
    ...
    fetch(`${API_URL}/TopUp`, {
        ...
        if (res.status === 200) {
            ...
            setModalVisible(false); // Close modal after successful top-up
            navigation.navigate('Wallet'); // Navigate to Wallet screen or refresh
        }
    }).catch((err) => {
