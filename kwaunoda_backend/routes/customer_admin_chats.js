...
CustomerAdminChatRouter.get('/customer_admin_chats/:trip_id', async (req, res) => {
    ...
    try {
        const results = await CustomerAdminChatsDbOperations.getCustomerAdminChatsByTripId(trip_id);
...
CustomerAdminChatRouter.put('/:customer_admin_chat_id', async (req, res) => {
    ...
    try {
        const result = await CustomerAdminChatsDbOperations.updateCustomerAdminChat(customer_admin_chat_id, updatedValues);
  ...
  CustomerAdminChatRouter.delete('/:id', async (req, res, next) => {
    try {
        ...
        let result = await  CustomerAdminChatsDbOperations.deleteCustomerAdminChat(id);
        res.json(result);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
