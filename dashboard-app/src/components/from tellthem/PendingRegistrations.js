...
const PendingReg = () => {
  ...
  const openModal = (image) => {
    if (image) {
      ...
      setSelectedImage(`${APILINK}${image}`);
      setModalIsOpen(true);
    }
  };
  ...
  const handleSearch = async () => {
    try {
      ...
      console.log("lemme see", `${APILINK}/uploads/${pending[0].profilePic}`);
  ...
  const verifyUser = async (driverId) => {
    try {
      const response = await fetch(
        `${APILINK}/driver/update_status/${driverId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        ...
        const result = await fetch(
          `${APILINK}/users/update_status/${driverId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              ...
              <div key={verification.driver_id} className="card">
                ...
                <div className="image-preview-container">
                  ...
                  {verification.driver_license_image && (
                    <img
                      src={`${APILINK}${verification.driver_license_image}`}
                      alt="Driver License"
                      onClick={() =>
                        openModal(verification.driver_license_image)
                      }
                      style={{
                        width: "100px",
                        borderRadius: "30px",
                  ...
                  {verification.id_image && (
                    <img
                      src={`${APILINK}${verification.id_image}`}
                      alt="ID"
                  ...
                  {verification.vehicle_license_image && (
                    <img
                      src={`${APILINK}${verification.vehicle_license_image}`}
                      alt="Vehicle License"
                      onClick={() =>
                        openModal(verification.vehicle_license_image)
                      }
                      style={{
                        width: "100px",
                        height: "auto",
                  ...
                  {verification.number_plate_image && (
                    <img
                      src={`${APILINK}${verification.number_plate_image}`}
                      alt="Number Plate"
                ...
                <div style={{ marginTop: '60px', marginBottom: '125px' }}>
                <button
                  className="verify-button"
                  onClick={() => verifyUser(verification.driver_id)}
                  style={{
                    marginTop: '135px',
                    width: "95%", // Make the button stretch full width
                    padding: "10px",
                    background: "green",
                    color: "white",
                    border: "none",
                ...
                <div className="info-row">
                  ...
                  <div>
                    {new Date(verification.signed_on).toLocaleDateString() ||
                      "N/A"}
                  </div>
                </div>
                <div className="info-row">
                ...
                <div className="info-row">
                  ...
                  <div>
                    {verification.house_number_and_street_name || "N/A"}
                  </div>
                </div>
                <div className="info-row">
                  <strong>City:</strong>
                ...
                <div className="info-row">
                  ...
                  <div>
                    {new Date(verification.dob).toLocaleDateString() || "N/A"}
                  </div>
                </div>
                <div className="info-row">
